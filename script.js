  ...
  ".inner-page-latest-project-slider",
  {
    ...
    breakpoints: {
      0: {
        effect: "slide",
        slidesPerView: 1,
        coverflowEffect: { rotate: 0, stretch: 0, depth: 0, scale: 1 },
      },
      992: {
        effect: "coverflow",
        slidesPerView: "auto",
...
function positionArrows() {
  ...
  // ── MOBILE: inline styles reset karo, CSS handle karega ──
  if (window.innerWidth < 992) {
    prev.style.left = "";
    next.style.left = "";
    prev.style.right = "";
    ...
    prev.style.top = "";
    next.style.top = "";
    return; // ← yahan return, upar wala duplicate tha
  }

  // ── DESKTOP: arrows ko side slides ke center par position karo ──
  const containerW = swiperContainer.offsetWidth;
  const center = containerW / 2;
  const arrowW = 48;
  const slideW = 300;
  const scale = 0.82;
  const sideW = slideW * scale; // 246px

  const prevCenter = center - slideW / 2 - sideW / 2;
  const nextCenter = center + slideW / 2 + sideW / 2;

  prev.style.left = prevCenter - arrowW / 2 + "px";
  next.style.left = nextCenter - arrowW / 2 + "px";
